 ...
 * 执行单个步骤（支持所有操作类型）
 */
async function executeStep(step, currentData, config, context) {
 ...
 * 单条数据录入流程
 */
async function processOneRecord(record, index, config, context) {
  console.log(`\n===== 第 ${index + 1} 条 =====`);

  for (const step of config.workflow) {
    try {
      await executeStep(step, record, config, context);
    } catch (err) {
...
function createConfigTemplate() {
  return {
    ...
    "workflow": [
      ...
      { "type": "click", "displace": "matchBtn", "delay": 1000,"double": true },
...
async function writingRules(inputArray, outputNodeTemplate) {
  ...
  for (let i = 0; i < jsonData.length; i++) {
    await processOneRecord(jsonData[i], i, config, context);
  }
...
module.exports = {
  ...
  description: '全能自动化工具：图像识别+表单自动录入+键盘鼠标模拟',
  notes: {
    node: '18.20.4',
    tips: [
      '1. 支持固定坐标点击和图像识别自动找图',
      '2. 所有流程通过config.json配置，无需修改代码',
      '3. 模板图片放在templates目录下，在配置中引用文件名',
      '4. 开启debugMode会保存识别截图到debug目录',
      '5. 运行前确保页面无遮挡，不要移动鼠标键盘'
    ]
  },
  input: {
    normExt: ['json', 'xlsx', 'png'],
    format: '上传：config.json(配置) + data.xlsx(数据) + templates目录(模板图片)'
  },
  output: {
    normExt: ['txt', 'json', 'xlsx', 'png']
  },
  rely: {
    'robotjs': '0.6.0',
