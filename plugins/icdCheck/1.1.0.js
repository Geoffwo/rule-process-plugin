 ...
 *
 * 输入约定：文件名 data.xlsx，第一个 sheet，A 列=ICD 编码、B 列=ICD 名称、C 列=icd_type，首行为表头。
...
module.exports = {
    ...
    notes: {
        node: '18.20.4',
        tips: [
            '输入约定：文件名 data.xlsx，第一个 sheet，A 列=ICD 编码、B 列=ICD 名称、C 列=icd_type、首行表头；无数据时自动输出输入模板',
    ...
    output: {
        normExt: 'xlsx文件',
        format: '预结果表：联合编码,联合名称,icd类型,ICD编码1,ICD名称1,ICD编码2,ICD名称2,匹配方式,关联得分,连接词'
    },
    rely: {
        'xlsx': '0.18.0'
    },
