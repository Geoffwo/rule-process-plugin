    ...
    "steps": [
      {
        "stepName": "管理员登录",
        "once": true, // 全局仅执行一次
        "url": "http://10.24.20.167:18080/api/v1/admin/login",
        "method": "POST",
      ...
      {
        "stepName": "批量创建用户",
        "url": "http://10.24.20.167:18080/api/v1/admin/users",
        "method": "POST",
        "headers": {
...
async function writingRules(inputArray, outputNodeTemplate) {
  ...
  return [
    {...outputNodeTemplate, fileName: 'result1', normExt:'json', content: JSON.stringify(content, null, 2)}
  ];
}
...
module.exports = {
  ...
  description: 'RAGFLOW定制-组合-批量注册-支持多步骤steps、once单次执行、全局共享上下文,json返回数据优化',
  notes: {
    node: '18.20.4'
  },
  input: {
