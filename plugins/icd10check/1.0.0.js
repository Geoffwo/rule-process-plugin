...
function createXlsxTemplate(outputPath){
  const headers = ['icd_code', 'icd_name', 'is_gray'];

  // 示例数据（可选，你也可以只创建带表头的空文件）
  const sampleData = [
    { icd_code: 'A16.201', 'icd_name': '肺结核','is_gray':0  },
    { icd_code: 'D48.901', 'icd_name': '瘤','is_gray':0  },
    { icd_code: 'A15.001', 'icd_name': '肺结核瘤','is_gray':0  }
  ];
...
function buildIndexes(rows) {
  ...
  rows.forEach((row, index) => {
    if (!nameToIdx.has(row.name))
      nameToIdx.set(row.name, index);
  });
...
function findExactSplits(union, preDatas, nameToIdx, rows, seen) {
  ...
  for (const item of preDatas) {
    ...
    const part2Idx = nameToIdx.get(split.part2Name);
...
function readExcel(file,sheet) {
  ...
  return xlsx.utils.sheet_to_json(worksheet)
      .map(r => ({ code: r.icd_code, name: r.icd_name, is_gray: r.is_gray }))
      .filter(r => r.name);
}
...
module.exports = {
  ...
  input: {
    normExt: 'xlsx文件',
    format: 'Excel：约定 data.xlsx，第一个 sheet，A 列编码 + B 列名称，首行表头'
  },
  output: {
    normExt: 'xlsx文件',
