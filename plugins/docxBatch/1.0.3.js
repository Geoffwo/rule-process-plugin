...
// 新增：图表循环块标记（XML中识别循环区域）
const CHART_LOOP1 = {
  start: 'GQ_LOOP_START', // 循环开始标记
  end: 'GQ_LOOP_END'       // 循环结束标记
};

const CHART_LOOP2 = {
  start: 'YD_LOOP_START', // 循环开始标记
  end: 'YD_LOOP_END'       // 循环结束标记
};

const CHART_LOOP3 = {
  start: 'GM_LOOP_START_THREE', // 循环开始标记
  end: 'GM_LOOP_END_THREE'       // 循环结束标记
};
...
async function generateReport(docxFile,outputNodeTemplate,contents,jsonFiles){
  ...
  const result = await generateMonthlyReport(tempDir, outputPath, month,jsonFiles);
 ...
 */
async function generateMonthlyReport(tempDir, outputPath, month,jsonFiles) {
  // 构建输出文件名和路径
  // const outputFileName = `青岛市电力看汽车制造业分析报告_${year}年${month}月.docx`;
  // const outputPath = path.join(outputDir, outputFileName);
  // 格式化月份（补零）
  ...
  try {
    ...
    // 改造：调用replaceXmlContent时传入xAxisData（供循环图表使用）
    for (const xmlFile of xmlFiles) {
      await replaceXmlContent(xmlFile, replaceData,xAxisData,jsonFiles);
    }
...
// 新增：获取单个地市的图表数据（近12个月指数趋势）
/**
 * 获取指定地市的图表数据
 * @param {string} area 地市名称（如市南）
 * @param {number} index 地市索引（用于生成唯一图表ID）
 * @param replaceData
 * @param xAxisData
 * @param jsonFiles
 * @returns {Object} 图表渲染所需数据
 */
function getAreaChartData(area, index, replaceData,xAxisData,jsonFiles) {
  ...
  keys.forEach((key, index)=>{
    ...
    const fieldName = 3400801 + index;
    result[fieldName] = city['电力景气指数'] || 0;

    if(index === keys.length - 1){
      areaJQ = city['电力景气指数'] || 0;
      areaYD = city['用电增长指数'] || 0;
      areaGM = city['规模增长指数'] || 0;
    }
  })
  ...
  // 电力景气指数状态
  const areaTsJQ = getJingQiStatus(areaJQ);
  // 用电增长指数状态
  const areaTsYD = getYongDianStatus(areaYD);
  // 规模增长指数状态
  const areaTsGM = getGuiMoStatus(areaGM);

  return {
    areaName: area,                  // 地市名称（如市南）
    ...replaceData,
    ...result,
    areaJQ:areaJQ,
    areaYD:areaYD,
...
/**
 * 替换XML中的地市图表循环块
 * @param {string} xmlStr XML原始内容
 * @param replaceData
 * @param xAxisData
 * @param jsonFiles
 * @param CHART_LOOP
 * @returns {string} 处理后的XML内容
 */
function processChartLoop(xmlStr,replaceData,xAxisData,jsonFiles,CHART_LOOP) {
  ...
  const { start, end } = CHART_LOOP;
  // 无循环块则直接返回原内容
  if (!xmlStr.includes(start) || !xmlStr.includes(end)) return xmlStr;

  // 提取循环模板（开始/结束标记之间的内容）
  const startIdx = xmlStr.indexOf(start) + start.length;
  const endIdx = xmlStr.indexOf(end);
  const loopTemplate = xmlStr.slice(startIdx, endIdx).trim();

  // 遍历地市生成每个地市的图表内容
  let loopContent = '';
  areas.forEach((area, index) => {
    let newXmlStr = loopTemplate;
    const chartData = getAreaChartData(area, index,replaceData,xAxisData,jsonFiles);
    ...
    keys.forEach(key => {
      ...
      const safePlaceholder = placeholder
          .replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
          .replace(/\\\*/g, '.*');
      newXmlStr = newXmlStr.replace(new RegExp(safePlaceholder, 'g'), chartData[key]);
    });

    loopContent += newXmlStr + '\n';
  });

  // 替换原循环块为生成的所有地市图表内容
  return xmlStr.replace(`${start}${xmlStr.slice(startIdx, endIdx)}${end}`, loopContent);
}
...
function processJsonData(jsonFile){
  ...
  // --------------- 赋值核心变量 ---------------
  // 电力景气指数状态
  const qingDaoTsJQ = getJingQiStatus(qingDao['电力景气指数']);
  // 用电增长指数状态
  const qingDaoTsYD = getYongDianStatus(qingDao['用电增长指数']);
  // 规模增长指数状态
  const qingDaoTsGM = getGuiMoStatus(qingDao['规模增长指数']);
  ...
  return {
    ...
    qingDaoTsJQ:qingDaoTsJQ,
    qingDaoTsYD:qingDaoTsYD,
    qingDaoTsGM:qingDaoTsGM,
    ...top3NumSortedData,
    ...top3JQSortedData,
    ...top3YDSortedData,
    ...top3GMSortedData,
...
/**
 * 电力景气指数状态判断
 * @param {number} index 景气指数数值
 * @returns {string} 状态描述
 */
function getJingQiStatus(index) {
  // 容错：非数字返回数据异常
  if (typeof index !== 'number' || isNaN(index)) return '。'
  if (index > 100) return ',呈现繁荣发展态势。';
  if (index >= 80) return ',呈现平稳发展态势。'; // 80-100区间（含80/100）
  if (index >= 10) return ',呈现偏弱发展态势。'; // 80-100区间（含80/100）
  return '。'; // <80
}

/**
 * 用电增长指数状态判断
 * @param {number} index 用电增长指数数值
 * @returns {string} 状态描述
 */
function getYongDianStatus(index) {
  if (typeof index !== 'number' || isNaN(index)) return '。';
  if (index > 100) return ',呈良好发展态势。';
  if (index >= 10) return ',呈平稳发展态势。'; // 80-100区间（含80/100）
  return '。'; // ≤80
}

/**
 * 规模增长指数状态判断
 * @param {number} index 规模增长指数数值
 * @returns {string} 状态描述
 */
function getGuiMoStatus(index) {
  if (typeof index !== 'number' || isNaN(index)) return '。';
  if (index > 100) return ',呈扩张态势。';
  if (index >= 10) return ',呈平稳态势。'; // 80-100区间（含80/100）
  return '。'; // ≤80
}
 ...
 * @param jsonFiles
 */
// 改造：替换XML内容（先处理循环块，再替换普通占位符）
async function replaceXmlContent(xmlPath, replaceData,xAxisData,jsonFiles) {
  try {
    let xmlStr = await fs.readFile(xmlPath, 'utf8');

    // 第一步：处理地市图表循环块
    xmlStr = processChartLoop(xmlStr, replaceData ,xAxisData,jsonFiles,CHART_LOOP1);
    xmlStr = processChartLoop(xmlStr, replaceData ,xAxisData,jsonFiles,CHART_LOOP2);
    xmlStr = processChartLoop(xmlStr, replaceData ,xAxisData,jsonFiles,CHART_LOOP3);
