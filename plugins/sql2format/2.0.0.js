...
function replaceSqlKeyParts(sql) {
  ...
  let result = sql
      // 1. 替换表名前缀 HISJN -> HIS
      .replace(/HISJN\.MEDICAL_ITEM_DUP_CHARGE_DICT/g, 'HIS.MEDICAL_ITEM_DUP_CHARGE_DICT')
      // 2. 字段替换 YLXMBM -> MICODE
      .replace(/YLXMBM/g, 'MICODE')
      // 3. 字段替换 YLXMMC -> listname
      .replace(/YLXMMC/g, 'listname')
      // 4. 剔除 AND YPBZ = '0' 条件（支持大小写、空格兼容）
      .replace(/\s*AND\s+YPBZ\s*=\s*'0'/gi, '')
      // 5. 替换业务表名
      .replace(/his\.SI_MEDI_ITEM_T/g, 'his.item_mapping_rmyy');
