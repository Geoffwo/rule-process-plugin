 ...
 */
async function runPageActions(page, actions, ctx, outputList, outputTpl) {
    let target = page;
            ...
            try {
                for (let stepIdx = 0; stepIdx < steps.length; stepIdx++) {
                    ...
                    if (Array.isArray(step.actions)) {
                        await runPageActions(page, step.actions, ctx, outputFiles, outputNodeTemplate);
                    }
...
module.exports = {
    ...
    description: 'Excel驱动puppeteer自动化，共享/独立双会话；once单次执行、iframe穿透；表单指令evalSetValue；变更：废弃step.type=screenshot；废弃顶层option内fullPage/selector/transparent，截图参数仅在screenshot Action中配置',
    notes: {
        node: '>=18.0.0',
        tips: `
【版本2.1重要变更】
