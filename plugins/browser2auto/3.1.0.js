        ...
        "steps": [
            ...
            },
            {
                ...
                "actions": [
                    ...
                    {"getData":"table tbody tr","collect":"td","value":"textContent","storeKey":"tableList"},
                    {"exportData":"tableList","value":"detail_{{reportId}}"}
...
async function* runPageActions(page, actions, ctx, outputTpl) {
    ...
    for (const action of actions) {
        ...
        if (action.fetchApi) {
            ...
            try {
                const respData = await target.evaluate(async (url, reqMethod, headersObj, bodyStr) => {
                    const opt = {
                        method: reqMethod,
                ...
                }, apiUrl, method, reqHeaders, bodyPayload);

                // 存入上下文
                ctx[storeKey] = respData;
                console.log(`[Action] fetchApi 请求成功，存入ctx.${storeKey} status:${respData.__status}`);
            } catch (err) {
        ...
        if (action.exportData) {
            ...
            console.log(`[Action] exportData 导出数据文件 ${fileName}.json`);
            continue;
        }
...
module.exports = {
    ...
    description: 'Excel驱动puppeteer自动化；双键(ctxKey/tabKey)驱动会话隔离与标签页复用；once成功后跳过、iframe穿透；迭代器实时导出；新增getData采集、fetchApi接口、exportData数据导出',
    notes: {
        node: '>=18.0.0',
        tips: `
【核心机制说明】
...
◆ getData DOM数据采集
{
  ...
  "collect":["td:nth-child(1)","td:nth-child(2)"] / "td",
  "value":"textContent",
  "storeKey":"tableList"
}
getData：父行选择器；value：取值属性 textContent / innerHTML / value / href
collect = 数组：多个独立选择器，el.querySelector；输出 [{idx0,idx1}]
collect = 字符串：批量选择器，el.querySelectorAll；输出 [{idx0,idx1}]
无collect：直接提取getData匹配元素属性；结果数组长度=1自动拆包为字符串
storeKey：数据存入ctx变量，全局模板 {{tableList}} 读取

◆ exportData JSON文件导出
{"exportData":"tableList","value":"detail_{{reportId}}"}
exportData：ctx内数据key；value：输出json文件名

◆ fetchApi 页面上下文发起请求（自动携带当前Cookie）
{
  "fetchApi":"接口地址",
  "method":"POST",
  "headers":{},
...
返回结构 {__success:boolean, __status:状态码, data:返回json}
...
◆ pageJs 在页面执行原生JS脚本
{"pageJs":"return document.title;"}

◆ humanWait 人工介入暂停，控制台回车继续
{"humanWait":"请完成验证码操作后回车"}

◆ updateCtx 捕获当前页面Cookie、localStorage、sessionStorage存入session
{"updateCtx":true}
