    ...
    for (let i = 0; i < totalFile; i++) {
        ...
        try {
            for await (const chunk of stream()) {
                ...
                // 偏移指针 替换 slice 减少内存拷贝开销累积
                let offset = 0; // 已处理偏移指针
                // 以 ; 作为语句结束符
                let semiIndex;
                while ((semiIndex = content.indexOf(';',offset)) !== -1) {// offset = 本次查找的起始偏移位置
                    // 截取当前完整 SQL
                    const sql = content.slice(offset, semiIndex + 1).trim();
                    // 指针后移，指向下一段开始
                    offset = semiIndex + 1;
                ...
                // 最后把未处理的尾部残段存起来
                remainChunk = content.slice(offset);
            }
...
module.exports = {
    ...
    description:'sqlServer-流式解析SQL，单文件独立统计，报错即时yield输出日志，无内存缓存防OOM,性能优化',
    notes:{
        node:'18.20.4',
    },
    input: {
