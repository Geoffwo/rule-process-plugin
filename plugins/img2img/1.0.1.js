...
const configUtils = {
  ...
  lfsFiles: [
    ...
    'vae_decoder/model.onnx',   // VAE解码器，将隐空间数据还原为图像（减噪）
    'vae_encoder/model.onnx'    // VAE编码器，将输入图片编码到隐空间（加噪）（图生图）
  ],
  /**
   * 拼接模型完整存放路径
  ...
  config:{
    ...
    denoisingStrength: 0.3      // 图生图降噪强度（0-1，值越大改变越大；无输入图片时忽略）
  }
};
...
async function loadSDSessions(modelRoot) {
  ...
  const paths = {
    ...
    vaeEncoder: path.join(modelRoot, 'vae_encoder/model.onnx'),//将输入图片编码到隐空间（图生图）
  ...
  const sessionOpts = {
    ...
    intraOpNumThreads: 3,   // 每个算子内部线程数=3，避免多线程抢占 (可以调小)
    interOpNumThreads: 3    // 算子间线程数=3 (可以调小)
  };
  ...
  const vaeEncoder = await ort.InferenceSession.create(paths.vaeEncoder,sessionOpts);

  // 实例化本地CLIP分词器
  const tokenizer = new ClipBPETokenizer(paths.vocab, paths.merges);//把文本转为 token ID 序列

  console.log('[LOG] 所有模型和分词器加载完成');
  return { tokenizer, textEncoder, unet, vaeDecoder, vaeEncoder };
}
...
// =======================
// 图片预处理：统一尺寸、转为原始RGB像素数据（图生图输入图片预处理）
// @param {Buffer} imgBuffer 原始图片二进制流
// @param {number} imgSize 目标尺寸（正方形边长）
// @returns {Promise<Buffer>} 处理后的原始像素Buffer（HWC, [0,255]）
// =======================
async function preprocessImage(imgBuffer, imgSize) {
  return await sharp(imgBuffer)
      .removeAlpha()                                  // 移除透明通道，确保3通道RGB
      .resize(imgSize, imgSize, { fit: 'cover' })     // 等比裁剪填充至目标尺寸
      .raw()                                          // 转为无格式原始像素数据
      .toBuffer();
}
...
// =======================
// SD核心推理函数：标准DDIM采样 + VAE解码，实现文生图/图生图
// @param {object} sessions 模型与分词器实例集合
// @param {object} configFile 配置文件节点（content为JSON字符串）
// @param {Buffer|null} initImageBuffer 输入图片buffer，存在则图生图，否则文生图
// @returns {Promise<Buffer>} 生成的PNG图片二进制流
// =======================
async function runSDInference(sessions,configFile,initImageBuffer) {
  const { tokenizer, textEncoder, unet, vaeDecoder, vaeEncoder } = sessions;
  const config = { ...configUtils.config, ...JSON.parse(configFile.content) };
  // 读取正向、反向提示词
  const {prompt,negativePrompt} = config;
  // 读取图像尺寸、采样步数、CFG强度、图生图降噪强度
  const { imgSize, inferenceSteps, guidanceScale, denoisingStrength } = config;
  ...
  if (initImageBuffer) {
    // ---- 图生图：输入图片编码到隐空间后，按降噪强度加噪 ----
    console.log('[LOG] 图生图模式，降噪强度:', denoisingStrength);
    // 图片预处理：缩放到 imgSize×imgSize，取原始RGB像素（HWC, [0,255]）
    const rawPixels = await preprocessImage(initImageBuffer, imgSize);
    // HWC[0,255] -> CHW[-1,1]，适配VAE编码器输入
    const encInput = new Float32Array(3 * imgSize * imgSize);
    for (let h = 0; h < imgSize; h++) {
      for (let w = 0; w < imgSize; w++) {
        for (let c = 0; c < 3; c++) {
          const srcIdx = (h * imgSize + w) * 3 + c;               // HWC
          const dstIdx = c * imgSize * imgSize + h * imgSize + w; // CHW
          encInput[dstIdx] = (rawPixels[srcIdx] / 255 - 0.5) / 0.5; // [0,255] -> [-1,1]
        }
      }
    }
    // VAE编码：像素 -> 隐空间
    const encTensor = new ort.Tensor('float32', encInput, [1, 3, imgSize, imgSize]);
    const vaeEncOut = await vaeEncoder.run({ sample: encTensor });
    const initLatentData = vaeEncOut[Object.keys(vaeEncOut)[0]].data;
    // 隐空间缩放：编码端乘VAE_SCALE（与解码端除VAE_SCALE对称）
    for (let i = 0; i < latents.length; i++) {
      latents[i] = initLatentData[i] * VAE_SCALE;
    }
    // 按降噪强度截取实际采样步数，确定起始时间步
    const initTimestep = Math.max(1, Math.round(inferenceSteps * denoisingStrength));
    const tStart = inferenceSteps - initTimestep;
    timesteps = timesteps.slice(tStart);
    const startT = timesteps[0];
    const sqrtAlphaStart = Math.sqrt(alphasCumprod[startT]);
    const sqrtOneMinusAlphaStart = Math.sqrt(1 - alphasCumprod[startT]);
    // 向初始隐空间加噪：x_t = sqrt(α_t)·x0 + sqrt(1-α_t)·noise
    for (let i = 0; i < latents.length; i++) {
      latents[i] = sqrtAlphaStart * latents[i] + sqrtOneMinusAlphaStart * gaussianRandom();
    }
    console.log('[LOG] 图生图起始时间步:', startT, '实际采样步数:', timesteps.length);
  } else {
    // ---- 文生图：纯随机高斯噪声 ----
    for (let i = 0; i < latents.length; i++) {
  ...
  console.log('[LOG] 初始隐空间 min:', lMin, ' max:', lMax, ' mean:', lSum/latents.length);
  ...
  const totalPixels = imgSize * imgSize;
  const totalElements = totalPixels * 3;
  const channelStride = totalPixels; // 单通道总元素数
...
// 单层扁平化循环，消除嵌套
  for (let i = 0; i < totalPixels; i++) {
    ...
    // R/G/B 三个通道依次映射
    // 通道0
    let val = (pixelData[i] + 1) / 2;
    val = Math.max(0, Math.min(1, val));
    outBuffer[hwcBase] = Math.floor(val * 255);

    // 通道1
    val = (pixelData[i + channelStride] + 1) / 2;
    val = Math.max(0, Math.min(1, val));
    outBuffer[hwcBase + 1] = Math.floor(val * 255);

    // 通道2
    val = (pixelData[i + channelStride * 2] + 1) / 2;
    val = Math.max(0, Math.min(1, val));
    outBuffer[hwcBase + 2] = Math.floor(val * 255);
  }
...
async function writingRules(inputArray, outputNodeTemplate) {
  ...
  // 2.1 从 inputArray 读取图片文件 buffer（存在则图生图，否则文生图）
  const imageExts = ['png', 'jpg', 'jpeg', 'webp', 'bmp'];
  const imageFile = inputArray.find(item => imageExts.includes(item.normExt));
  //     图片类文件由规则引擎以 buffer 编码读取，item.content 即为 Buffer
  const initImageBuffer = imageFile ? imageFile.content : null;

  const result = [];

  console.log(`开始生成图片...`, imageFile ? `图生图(${imageFile.base})` : '文生图');
  const timestamp = getTimestamp(); // 获取时间戳
  try {
    // 3. 执行推理（文生图/图生图）
    const resultBuffer = await runSDInference(sessions, configFile, initImageBuffer);
...
/*
文生图：随机高斯噪声 → 迭代去噪 (DDIM + UNet) ← 文本嵌入 → VAE解码 → PNG
图生图：输入图片 → VAE编码 → 乘VAE_SCALE → 按降噪强度加噪 → 迭代去噪(部分步数) → VAE解码 → PNG

文本提示词 → 分词器 → token IDs (77) → text encoder → 文本嵌入 (1,77,768)
                                                          ↓
         文生图: 随机高斯噪声        图生图: 图片→VAE编码→加噪(按denoisingStrength截取步数)
                          ↓ 迭代去噪 (DDIM + UNet) ← 文本嵌入 ↓
                                          去噪后隐变量 (1,4,64,64)
                                                          ↓
...
module.exports = {
  ...
  description: '基于 ONNX Runtime + Stable Diffusion v1.5 ONNX 实现文生图/图生图，全程离线、零第三方分词依赖（性能优化-常规电脑实现运行-增加时间降低CPU线程）',
  notes: {
    node: '18.20.4',
    model: 'onnx-community/stable-diffusion-v1-5-ONNX',
    tips: '需提前安装Git、Git LFS；首次运行自动下载模型，后续离线使用；inputArray含图片则图生图(由denoisingStrength控制改变程度)，否则文生图'
  },
  error: {
    'model-download': {
  ...
  input: {
    normExt: 'json',
    description: 'config.json配置提示词；可选放入png/jpg图片则触发图生图'
  },
  output: {
    normExt: 'png、txt',
