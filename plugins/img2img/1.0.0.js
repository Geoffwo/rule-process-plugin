...
const configUtils = {
  ...
  // 推理、提示词相关配置项(默认配置)
  config:{
    prompt: "8-bit pixel art, retro game sprite, pixelated, sharp pixel edges, clean pixel blocks, limited color palette, game art, flat shading, pixel graphics",// 正向提示词：指定生成复古8位像素游戏画风
    negativePrompt: "photograph, realistic, smooth gradient, blurry, pencil sketch, painting, 3d render, textures, complex shadows, anti-aliasing, high detail, cinematic, watercolor",// 反向提示词：屏蔽写实、模糊、畸形、3D等无效画面
    imgSize: 512,                // 生成图像分辨率 512*512（SD1.5标准尺寸）
    inferenceSteps: 20,          // DDIM采样迭代步数，步数越高画质越好、速度越慢
    guidanceScale: 7.5,         // CFG引导强度，控制画面贴合提示词的程度
    denoisingStrength: 0.4      // 图生图降噪强度（0-1，值越大改变越大；无输入图片时忽略）
  }
};
...
async function loadSDSessions(modelRoot) {
  ...
  const [textEncoder, unet, vaeDecoder, vaeEncoder] = await Promise.all([
    ...
    ort.InferenceSession.create(paths.vaeEncoder)
  ]);
...
async function runSDInference(sessions,configFile,initImageBuffer) {
  ...
  if (initImageBuffer) {
    ...
    // ✅【标准DDIM图生图时间步生成，修复原先错误切片逻辑】
    const tStart = Math.round(totalSteps * denoisingStrength);
    timesteps = [];
    for (let i = inferenceSteps - 1; i >= 0; i--) {
      const ratio = i / (inferenceSteps - 1);
      const t = Math.round(tStart - ratio * tStart);
      timesteps.unshift(t);
    }

    const sqrtAlphaStart = Math.sqrt(alphasCumprod[tStart]);
    const sqrtOneMinusAlphaStart = Math.sqrt(1 - alphasCumprod[tStart]);
    // 向初始隐空间加噪：x_t = sqrt(α_t)·x0 + sqrt(1-α_t)·noise
    for (let i = 0; i < latents.length; i++) {
      latents[i] = sqrtAlphaStart * latents[i] + sqrtOneMinusAlphaStart * gaussianRandom();
    }
    console.log('[LOG] 图生图起始时间步:', tStart, '实际采样序列:', timesteps);
  } else {
    // ---- 文生图：纯随机高斯噪声 ----
    for (let i = 0; i < latents.length; i++) {
...
module.exports = {
  ...
  description: '基于 ONNX Runtime + Stable Diffusion v1.5 ONNX 实现文生图/图生图，全程离线、零第三方分词依赖',
  notes: {
    node: '18.20.4',
