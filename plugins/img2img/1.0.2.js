...
const configUtils = {
  ...
  lfsFiles: [
    ...
    'vae_encoder/model.onnx',   // VAE编码器，将输入图片编码到隐空间（图生图）
  ],
  /**
   * 拼接模型完整存放路径
  ...
  config:{
    ...
    denoisingStrength: 0.7,     // 图生图降噪强度（0-1，值越大改变越大；无输入图片时忽略）
...
async function* runSDInference(sessions, configFile, initImageBuffer) {
  ...
  // ========== 3. 初始化隐空间 ==========
  const VAE_SCALE = 0.18215;                              // SD官方VAE隐空间缩放系数
...
async function* writingRules(inputArray, outputNodeTemplate) {
  ...
  const imageFile = inputArray.find(item => item.encode === 'buffer' && imageExts.includes(item.normExt));
  ...
  try {
    // 3. 执行推理（文生图/图生图，迭代器逐步产出 debug 和 result）
    const runSD= runSDInference(sessions, configFile, initImageBuffer)
...
module.exports = {
  ...
  description: '基于 ONNX Runtime + Stable Diffusion v1.5 ONNX 实现文生图/图生图，全程离线、零第三方分词依赖（增加调试过程，渲染每一步的图片，展示【噪点->图片】链路）',
  notes: {
    node: '18.20.4',
