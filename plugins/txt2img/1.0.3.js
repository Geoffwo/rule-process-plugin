...
async function* runSDInference(sessions, configFile) {
  ...
  // - 第三个参数：[1, 77] → 张量形状（dims），1=批次大小(batch)，一次只处理1条文本；77=单条文本固定77个Token（这是 CLIP text_encoder 强制要求的输入形状）
  ...
  const textEmbeds = await textEncoder.run({ input_ids: inputIds });//加载text_encoder.onnx（CLIP 文本编码器）onnx模型，输入参数（Netron查看onnx入参）
  const negTextEmbeds = await textEncoder.run({ input_ids: negInputIds });//把 77 个 Token ID 转换成模型能理解的语义向量（文本嵌入）
  ...
  const timesteps = [];
  for (let i = inferenceSteps - 1; i >= 0; i--) {//在 1000 步时间轴上，均匀选 N 个点，从最噪（999）到最干净（0）排好
    const t = Math.round(totalSteps - 1 - (i / (inferenceSteps - 1)) * (totalSteps - 1));//这个公式可以让AI解释一下
  ...
  for (let i = 0; i < timesteps.length; i++) {
    ...
    // UNet推理：预测当前时间步的噪声
    const outputs = await unet.run({// 正向提示词推理「如何朝着你想要的画面去噪」
      sample: latentTensor,// 当前隐空间噪声图
      timestep: new ort.Tensor('float32', [t], [1]),// 当前时间步
      encoder_hidden_states: textEmbed  // 正向文本特征 [1,77,768]
    });
    const negOutputs = await unet.run({// 反向提示词推理「如何朝着你不想要的画面去噪」
      sample: latentTensor,
      timestep: new ort.Tensor('float32', [t], [1]),
      encoder_hidden_states: negTextEmbed // 反向文本特征
    });
...
module.exports = {
  ...
  description: '基于 ONNX Runtime + Stable Diffusion v1.5 ONNX 实现文生图，全程离线、零第三方分词依赖（增加注释）',
  notes: {
    node: '18.20.4',
    ...
    choise: '如果电脑性能足够，可以使用1.0.4，加速方案，自行平衡判断'
  },
  error: {
    'model-download': {
