...
<script>
  ...
  function copyFileList(format) {
    ...
    if (format === 'json') {
      ...
      // 纯文件名清单（每行一个）
      text = files.map(f => f.name+'.'+f.ext).join('\\n');
      label = '文件名';
...
module.exports = {
    ...
    description: '将输入的文件数组拼接成树结构，输出可展开/收缩的 HTML 文件，文件支持点击打开/复制路径，并支持批量复制对象数组/文件名/完整路径',
    notes: {
        node: '18.20.4'
    },
    input: {
    ...
    output: {
        normExt: 'html',
        format: '可展开/收缩的文件树结构 HTML 文件，支持点击打开/复制路径/批量复制'
    }
};
